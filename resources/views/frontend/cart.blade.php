                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="ps-sm-3">
                                            ...
                                            <small class="d-block text-muted">Size : {{ $item->options->Size }}</small>
                                        </div>
                                    </td>
                                    <!-- Details -->
                                    ...
                                    <td>
                                        <div class="px-3 d-flex align-items-center">
                                            <button class="btn btn-sm update-cart-qty" data-type="decrease"
                                                data-id="{{ $item->rowId }}">-</button>

                                            <span class="mx-2 fw-bold item-qty"
                                                id="item-qty-{{ $item->rowId }}">{{ $item->qty }}</span>

                                            <button class="btn btn-sm update-cart-qty" data-type="increase"
                                                data-id="{{ $item->rowId }}">+</button>
                                        </div>
                                        <div class="px-3">
                                            <span class="small text-muted">{{ $item->qty }} @
                                                {{ $item->price }}/Item</span>
                                        </div>
                                        <div class="px-3">
                                            <span class="small text-muted">Discount:
                                                {{ $item->options->Variantdata->discount }} % </span>
                                        </div>
                                        <div class="px-3">
                                            <span class="small text-muted">Actual Price:
                                                {{ $item->options->Variantdata->price }}</span>
                                        </div>
                                    </td>
                                    <!-- /Qty -->
                                    ...
                                    <td class="f-h-0">
                                        <div class="d-flex justify-content-between flex-column align-items-end h-100">
                                            ...
                                            @endphp
                                            <p class="fw-bolder mt-3 m-sm-0">Rs. {{ $ItemTotal }}</p>
                                        </div>
                                    </td>
                                    <!-- /Actions -->
                ...
                <div class="bg-dark p-4 p-md-5 text-white">
                    ...
                    <div class="py-3 border-bottom-white-opacity">
                        <div class="d-flex justify-content-between align-items-center flex-column flex-sm-row">
                            ...
                            <p class="mt-3 m-sm-0 fs-5 fw-bold">Rs.{{ $subtotal }}<span><small> + Shipping
                                    </small></span></p>
                        </div>
                    </div>
                    ...
                    <!-- Checkout Button-->
                    <a href="{{ route('checkout.page') }}" class="btn btn-white w-100 text-center mt-3" role="button"><i
                            class="ri-secure-payment-line align-bottom"></i> Proceed to checkout</a>
                    {{-- <a href="{{ route('checkout.page') }}" class="btn btn-orange w-100 text-center mt-3" role="button"><i
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            $('#item-qty-' + rowId).text(response.qty);

                            // Update item total
                            $('tr').find('[data-rowid="' + rowId + '"] .item-total').text(
                                'Rs. ' + response.itemTotal);

                            // Update subtotal
                            $('#cart-subtotal').text('Rs. ' + response.subtotal);
