...
@section('main-container')
    <style>
        ...
        /* Updated CSS for bottom-right position */
        .simple-toast {
            position: fixed;
            bottom: 20px;
            /* Changed from top: 20px */
            right: 20px;
            padding: 15px 20px;
            border-radius: 5px;
            color: white;
        ...
        <!-- Category Top Banner -->
            ...
            <div class="row">
                                        ...
                                        @foreach ($categorydata as $category)
                                            <li class="mb-2"><a
                                                    ...
                                                    onclick="loadProducts({{ $category->id }})"><span><i
                                                        ...
                                                        {{ $category->name }}</span>
                                                    <span class="text-muted ms-4">({{ $category->products->count() }})
                                                    </span>
                                                </a>
                                            </li>
                                        @endforeach
                                ...
                                <div id="filter-type" class="collapse show">
                                    ...
                                    <div class="filter-options">
                                        @foreach ($producttype as $type)
                                            <div class="form-group form-check mb-0">
                                                <input type="checkbox" class="form-check-input" id="filter-type">
                                                <label
                                                    class="form-check-label fw-normal text-body flex-grow-1 d-flex justify-content-between"
                ...
                <div class="col-12 col-lg-9">
                    ...
                    <div id="product-list" class="row g-4 mb-5">
                                ...
                                <div class="card position-relative h-100 card-listing hover-trigger">
                                    <div class="card-header">
                                        ...
                                        {{-- <div class="card-actions" id="quickadd">
                                            <a href="{{ route('quick.add', ['product_id' => $product->id]) }}">
                                                <span
                    ...
                    {{-- <div class="modal fade" id="addedToCartModal" tabindex="-1" aria-labelledby="addedToCartModalLabel"
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="addedToCartModalLabel">Product Added to Cart!!</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="ms-0">
                                            <h6 id="modal-product-name" class="mb-0"></h6>
                                            <p id="modal-product-price" class="text-success mb-0"></p>
                                            <p id="modal-product-discount" class="text-success mb-0"></p>
                                            <p id="modal-product-actualprice" class="text-success mb-0"></p>
                                        </div>  
                                    </div>
                                </div>
                                <div class="modal-footer">
...
@push('scripts')
    ...
    {{-- script for simple quick add button which shows a toast message on adding to cart --}}
    {{-- <script>
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            // Quick Add button opens modal with product details
            $('.quickadd-btn').on('click', function(e) {
                e.preventDefault();
                productId = $(this).data('product-id');

                $.ajax({
                    url: '/quick-add/' + productId, // this returns product + variants
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (product.variants && product.variants.length > 0) {
                                ...
                                product.variants.forEach(function(variant) {
                                    ...
                                    colorMap[variant.color].sizes.push(variant.size);
                                });
            ...
            <div class="color-option" 
                data-color="${color}" 
                data-image="${colorMap[color].image}" 
                data-sizes='${JSON.stringify(colorMap[color].sizes)}'
                style="width:30px;height:30px;border-radius:50%;background:${color};cursor:pointer;border:2px solid #ccc;">
            </div>
        `);
                                });
                            }
            ...
            // Select color → update image + available sizes
            // $(document).on('click', '.color-option', function() {
            //     selectedColor = $(this).data('color-id');
            //     let image = $(this).data('image');
            //     let sizes = JSON.parse($(this).data('sizes'));
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            // Optional: refresh offcanvas mini cart
                            $('#offcanvasbody').load(location.href + ' #offcanvasbody > *');
                        } else {
